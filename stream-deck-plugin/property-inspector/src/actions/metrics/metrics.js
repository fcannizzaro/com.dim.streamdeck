...
import Triumphs from './triumphs.png';
...
const metrics = [
  ...
  [Triumphs, '#baa045', 'triumphs'],
  [BattlePass, '#712398', 'battlePass'],
];

export const Metrics = () => {
  const [settings, setSettings] = useSettings();
  const metricsButtons = metrics.map(([icon, color, metric], i) => (
    <IconButtonBox key={i}>
      {color && (
        <IconButton
          icon={icon}
  ...
  return (
    <SettingsBox>
      <SettingsButtons>{metricsButtons}</SettingsButtons>
    </SettingsBox>
  );
};
