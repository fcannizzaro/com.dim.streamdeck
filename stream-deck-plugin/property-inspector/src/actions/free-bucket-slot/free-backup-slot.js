...
export const FreeBackupSlot = () => {
  const [settings, setSettings] = useSettings();
  const slotsButtons = slots.map(([icon, bucket], i) => (
    <IconButtonBox key={i}>
      <IconButton
        color='#debf7a'
        icon={icon}
        selected={settings['bucket'] === bucket}
        onClick={() => setSettings({ bucket })}
      />
    </IconButtonBox>
  ));
  return (
    <SettingsBox>
      <SettingsButtons>{slotsButtons}</SettingsButtons>
    </SettingsBox>
  );
};
